...
1. [ rvalue reference](#rvaluereference)
...
## rvalue reference <a name="rvaluereference"></a>

```move``` semantics relies on rvalue reference

*lvalue* is an expression whose address can be taken. lvalue provides a (semi) permanent piece of memory. can assignment to a lvalue

*rvlaue*: an expression is an rvalue if it results in a temporary object
...
get_value() is a rvalue. 


*rvalue reference* is a reference that will bind only to a temporary object. Why having it? THe biggest advantage is to create move constructor and assignment constructor based on a temporary object. Reduce the overhead to copy or clone from an old object. 

*std::move* is a typecasting that covers a lvalue into a rvlaue
